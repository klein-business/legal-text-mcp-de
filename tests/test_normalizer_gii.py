...
def test_gii_parser_extracts_paragraph_and_subdivision(tmp_path):
    ...
    source = {
        "source_metadata": {"source_path": "ddg"},
        "source_url": "https://www.gesetze-im-internet.de/ddg/xml.zip",
...
def test_gii_parser_extracts_egbgb_article_child(tmp_path):
    ...
    with zipfile.ZipFile(zip_path, "w") as archive:
        archive.writestr("law.xml", xml)
    source = {
        "source_metadata": {"source_path": "bgbeg"},
        "source_url": "https://www.gesetze-im-internet.de/bgbeg/xml.zip",
        "source_kind": "gesetze-im-internet",
    }
