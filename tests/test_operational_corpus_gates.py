...
from legal_text_mcp_de.legal_texts.state_law_inventory import FIXED_STATE_CODES
...
PRIVACY_SCOPE_SEED = ROOT / "src" / "legal_text_mcp_de" / "legal_texts" / "data" / "privacy_scope_seed.v1.json"
...
def test_gitignore_and_release_gate_hygiene():
    ...
    assert not any(line.strip() == "tests/fixtures/" for line in gitignore.splitlines())
    ...
    assert "tests/test_source_matrix_live.py" not in verify_release.selected_tests()
...
def test_release_gate_live_source_matrix_is_explicit_opt_in(monkeypatch):
    ...
    assert "tests/test_source_matrix_live.py" in verify_release.selected_tests()
