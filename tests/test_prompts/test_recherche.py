...
def test_recherche_contains_topic_and_key_terms():
    app = create_mcp_app()
    result = _get_prompt(app, "recherche", {"topic": "Mietrecht Kündigung"})
    messages = result.messages
    assert len(messages) == 1
    body = _text(messages[0])
    assert "Mietrecht Kündigung" in body
    assert "search_laws" in body
