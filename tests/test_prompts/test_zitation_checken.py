...
def test_zitation_checken_contains_citation_and_key_terms():
    app = create_mcp_app()
    result = _get_prompt(app, "zitation_checken", {"citation": "§ 433 Abs. 1 BGB"})
    messages = result.messages
    assert len(messages) == 1
    body = _text(messages[0])
    assert "§ 433 Abs. 1 BGB" in body
    assert "resolve_citation" in body
    assert "Stand-Datum" in body
